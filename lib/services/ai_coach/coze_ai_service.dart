...
class CozeAIService {
  ...
  CozeAIService({
    ...
    required LoggerService logger,
        ...
        _cache = cache,
        _logger = logger {
    _parser = AiResponseParser(logger: _logger);
    _configureDio();
  }
  ...
  Future<AICoachResult> analyzeSession(
    ...
    try {
      ...
      // 3. 解析 AI 建议为结构化结果
      final result = _parser.parseCoachResult(
        rawText: aiResponse,
        source: 'coze',
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is ResponseBody) {
          final streamText =
              await utf8.decoder.bind(responseData.stream).join();
          final answer = _parser.extractAnswerFromSse(streamText);
  ...
  @visibleForTesting
  String extractAnswerFromSseForTest(String streamText) {
    return _parser.extractAnswerFromSse(streamText);
  }
...
''';
  ...
  String _buildPeriodPrompt(
    ...
    final totalArrows =
        recentSessions.fold<int>(0, (sum, s) => sum + s.arrowCount);
...
''';
  ...
  double _calculateAverage(List<TrainingSession> sessions) {
    ...
    final total =
        sessions.fold<double>(0.0, (sum, s) => sum + s.averageArrowScore);
